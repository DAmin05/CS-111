public class Communities {
    ...
    public static double highestPopulationWithHealthIssue (double[][] data, char healthIssue) 
    {
        ...
        for (double[] community_data : data)
        {
            double cases = community_data[index];
            if (cases > max_cases)
            {
                max_cases = cases;
            }
        }
        return max_cases; // replace this line
    }
    ...
    public static int numberOfCommunities (double[][] data, int columnIndex, double percentageThreshold, boolean isDisadvantage) {
        ...
        int count = 0;
         for(int i=0 ; i<data.length ; i++)
         {
            if((data[i][columnIndex]*100 >= percentageThreshold) && (data[i][21] == true_false)){
                count++;
            }
         }
    ...
    public static double largestRacialDemographic(double[][] data, int community) {
        ...
        for (int i=0 ; i<8 ; i++)
        {
            if(data[community][i] > maximum){
                maximum = data[community][i];
            }
        }

        return maximum*100; // replace this line
    }
    ...
    public static int mostPopulatedDisadvantagedWithLowIncome(double[][] data) {
        int index_mostpopulated = 0;
        int largest = 0;
        for (int i=0 ; i < data.length ; i++)
        {
            if((data[i][ECON_LOW_INCOME] == 1) && (data[i][ECON_DISADVANTAGED] == 1))
            {
                if(data[i][TOTAL_POPULATION] > largest)
                {
                    largest = (int) data[i][TOTAL_POPULATION];
                    index_mostpopulated = i;
                }
            }
        }
        return index_mostpopulated; // replace this line
    }
