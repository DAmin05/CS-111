...
public class DNA {
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        /* WRITE YOUR CODE HERE */
        String firstLine = StdIn.readLine();
        int s = Integer.parseInt(firstLine);
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        int counter = 0;
        int maxrepeats = 0;
        for(int i=0 ; i<sequence.length()-(STR.length()-1) ; i++)
        {
        ...
        STR num_repeats = new STR(STR, maxrepeats);
        return num_repeats; // update the return value
    }
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        STR[] S1_STR = new STR[allSTRs.length];
        STR[] S2_STR = new STR[allSTRs.length];

        for(int i=0 ; i<allSTRs.length ; i++)
        {
            String STR = allSTRs[i];
            STR s1 = findSTRInSequence(sequence1 , STR);
            STR s2 = findSTRInSequence(sequence2 , STR);
            S1_STR[i] = s1;
            S2_STR[i] = s2;
        }

        profile.setS1_STRs(S1_STR);
        profile.setS2_STRs(S2_STR);
    }
    ...
    public boolean identicalSTRs ( STR[] s1, STR[] s2 ) {
        ...
        {
            if (!(((s1[i].getSTR()).equals(s2[i].getSTR())) && (s1[i].getRepeats() == s2[i].getRepeats()))) //!(((s1[i].getSTR()).equals(s2[i].getSTR())) && (s1[i].getRepeats() == s2[i].getRepeats()))
            {
                return false;
            }
        }
